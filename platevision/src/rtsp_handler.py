...
class RTSPHandler:
    ...
    def _handle_person_detection(self, person, full_results, frame=None, annotated=None):
        ...
        save_images = bool(self.config_manager.get('people', 'image_history_enabled'))
        if not person.get('counted') and not save_all and not save_images:
            return
        event = {
        ...
        if saved:
            try:
                ...
                logger.info('RTSP Person gespeichert: track=%s counted=%s direction=%s images=%s', saved.get('track_id'), saved.get('counted'), saved.get('direction'), ','.join(imgs.keys()) if imgs else '-')
            except Exception:
                pass
            try:
                from flask_socketio import emit
